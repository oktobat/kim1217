...
<c:set var="pageNums" value="${pageNums }" />
<c:set var="total_page" value="${Math.ceil(totArticles/amount)}" />
<c:set var="tpgn" value="${Math.ceil(total_page/pageNums)}" />
<c:set var="start" value="${(pageNum-1)*amount }" />
<c:set var="last" value="${pageNum*amount }" />
<c:set var="number" value="${totArticles - start}" />
...
<div class="cl2" style="text-align:center; margin:30px">
    ...
    <c:if test="${pgn != 1}">
        <a href="javascript:;" onclick="pageGrouping(${pgn}, 1, 0, 0)" class="prevpage  pbtn">
            이전
        </a>
    </c:if>

	
	    <c:forEach var="i" begin="${(pgn-1) * pageNums + 1}" end="${(pgn * pageNums < total_page) ? pgn*pageNums : total_page}">
	        <c:choose>
	            <c:when test="${page == i }">
	                <a href="javascript:;">
	            ...
	            <c:otherwise>
	                <a href="javascript:;" onclick="paging(${pgn}, ${i})">
    ...
    <c:if test="${pgn != tpgn}">
        <a href="javascript:;" onclick="pageGrouping(${pgn}, 2, ${tpgn}, ${total_page})" class="nextpage  pbtn">
            다음
        </a>
    </c:if>
    <c:if test="${page<total_page }">
	    <a href="javascript:;" onclick="pageGrouping(${tpgn}, 2, ${tpgn}, ${total_page})"  class="lastpage  pbtn">
	        맨마지막
	    </a>
    </c:if>
</div>
